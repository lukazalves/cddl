...
import br.pucrio.inf.lac.mhub.s2pa.technologies.ble.devices.GenericActuator;
...
public class BLETechnology extends ResultReceiver implements Technology {
	...
	private ConcurrentHashMap<String, BluetoothGatt> mDeviceGatt;
	...
	public boolean initialize() {
		...
		mDeviceGatt = new ConcurrentHashMap<>();
	...
	public void writeSensorValue( String macAddress, String serviceID, Object value ) {
		...
		BluetoothGatt gatt = mConnectedDevices.get(macAddress).mGatt;
		final TechnologyDevice td = mDeviceModules.get(gatt.getDevice().getName());
		TechnologySensor ts = td.getServiceByName(serviceID);

		final BLEActuationMessage m = new Gson().fromJson((String) value, BLEActuationMessage.class);

		int connectionState = mBluetoothManager.getConnectionState(gatt.getDevice(), BluetoothProfile.GATT);
		if (connectionState != BluetoothGatt.STATE_CONNECTED) {
			gatt = gatt.getDevice().connectGatt(ac, false, new MobileObject(gatt.getDevice()));
		}

		try {
			// obtem os atributos genéricos e escreve o valor dscrito no driver
			// final BluetoothGattService service = gatt.getService(UUID.fromString(m.getServiceUUID()));
			final BluetoothGattService service = gatt.getService(ts.getService());
			if (service == null)
				throw new Exception("não foi possível obter o serviço: " + serviceID);

			final BluetoothGattCharacteristic characteristic = service.getCharacteristic(UUID.fromString(m.getCharacteristicUUID()));
			if (characteristic == null)
				throw new Exception("não foi possível obter a characterística: " + m.getCharacteristicUUID());

			characteristic.setValue(m.getCommand());

			while (!gatt.writeCharacteristic(characteristic)) ;

			/**
			 * teste para verificar se esperando um tempo após transmitir cada mensagem o BB-8 responde
			 * em caso positivo, devo incluir um campo "delay" para que fabricante defina o tempo que deve esperar entre 1 mensagem e outra.
			 */
			Thread.sleep(100);
		}
		catch (Exception e) {
    ...
    private void getDeviceStructureAndConnect( final BluetoothDevice device ) {
        ...
        if( deviceName == null ) {
            ...
            MHubEventBus.getDefault().post( model );
        }
    } 
        ...
        switch( resultCode ) {
            case AdaptationService.SUCCESS:
                ...
                if( componentInstance != null ) {
                    ...
                    BluetoothGatt gatt = device.connectGatt( ac, false, new MobileObject( device ) );

                    // guarda o BluetoothGatt do atuador conectado
                    if (componentInstance instanceof GenericActuator) {
                    	mDeviceGatt.putIfAbsent(device.getName(), gatt);
					}
                } else {
    ...
    public boolean writeCharacteristic(String deviceName, String serviceUUID, String characteristicUUID, byte[] value) throws Exception {
    	if (!mDeviceGatt.containsKey(deviceName))
    		throw new Exception("Nenhum BluetoothGatt para o dispositivo " + deviceName);

    	BluetoothGatt gatt = mDeviceGatt.get(deviceName);

    	int connectionState = mBluetoothManager.getConnectionState(gatt.getDevice(), BluetoothProfile.GATT);
    	if (connectionState != BluetoothGatt.STATE_CONNECTED) {
			gatt = gatt.getDevice().connectGatt(ac, false, new MobileObject(gatt.getDevice()));
			mDeviceGatt.put(deviceName, gatt);
		}

		// obtem os atributos genéricos e escreve o valor dscrito no driver
		BluetoothGattService service = gatt.getService(UUID.fromString(serviceUUID));
		if (service == null)
			throw new Exception ("não foi possível obter o serviço: " + serviceUUID);

		BluetoothGattCharacteristic characteristic = service.getCharacteristic(UUID.fromString(characteristicUUID));
		if (characteristic == null)
			throw new Exception("não foi possível obter a characterística: " + characteristicUUID);
